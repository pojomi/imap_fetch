...
size_t decode_64(Str buff_d, Str buff_s, size_t *leftover)
{
    ...
    *leftover = 0;
    ...
    while (i < buff_s.len)
    {
        if (i + 3 < buff_s.len)
        {
            if (buff_s.data[i] == '\r' || buff_s.data[i] == '\n')
            {
                i++;
                continue;
            }
            if (buff_s.data[i+2] == '=' && buff_s.data[i+3] == '=')
            {
                // Decode double padded
                b64v1 = get_b64_value(buff_s.data[i]);
                if (b64v1 == -1) 
                {
                    printf("Failed on first byte from double padded block\n");
                    return -1;
                }
                b64v2 = get_b64_value(buff_s.data[i+1]);
                if (b64v2 == -1) 
                {
            ...
            if (buff_s.data[i+3] == '=')
            {
                ...
                b64v3 = get_b64_value(buff_s.data[i+2]);
                if (b64v3 == -1) 
                {
            ...
            b64v4 = get_b64_value(buff_s.data[i+3]);
            if (b64v4 == -1) 
            {
...
size_t decode_qp(Str buff, size_t *leftover)
{
    // Read position
    size_t i = 0;
    // Write position
    ...
    while (i < buff.len)
    {
        if (buff.data[i] == '=')
        {
            ...
            // For \r, disregard
            if (i + 2 < buff.len && buff.data[i + 1] == '\r' && buff.data[i + 2] == '\n')
            {
                i += 3;
                continue;
            }
